...
const InvestorRegistrationForm = () => {
  const title = ["Personal Details", "Wallet", "Terms", "Completed"];
  const { step, setStep, data, setData } = useContext(InvestorContext);
  ...
  const handleSubmit = (e) => {
    ...
    const investorObject = JSON.stringify(data);
    const config = {
      headers: {
    ...
    setLoading(true);
    axios
      .post("http://localhost:4000/api/investor", investorObject, config)
      .then((res) => {
        updateRegistrationStatus(res.data.success);
        nextStep();
      })
      .catch((error) => {
        console.log(error);
